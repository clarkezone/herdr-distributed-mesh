# Advanced Herdr Mesh operator guide

This reference covers explicit process roles, transport state, and low-level
target/retry parameters. For ordinary setup and operation, start with the
[operator guide](operator-guide.md). Do not mix independently running legacy
roles with managed setup or copy their identity state into the managed daemon.
